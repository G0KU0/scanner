...
async def create_run(user_id: str, keyword: str, total: int):
    run = {
        "user_id": user_id, "keyword": keyword, "total": total, "checked": 0, "hits": 0, "custom": 0, "bad": 0, "retries": 0,
        "status": "running", "hit_lines": [], "custom_lines": [], "hit_details": [], "custom_details": [],
        "hits_url": None, "custom_url": None, "started_at": datetime.now(timezone.utc), "finished_at": None
    }
    result = await runs_collection.insert_one(run)
    return str(result.inserted_id)

async def delete_old_runs(user_id: str, keep_run_id: str):
    try: await runs_collection.delete_many({"user_id": user_id, "_id": {"$ne": ObjectId(keep_run_id)}})
    except: pass

async def get_run(run_id: str):
    try: return await runs_collection.find_one({"_id": ObjectId(run_id)})
    except: return None

async def update_run_stats(run_id: str, stats: dict):
    try: await runs_collection.update_one({"_id": ObjectId(run_id)}, {"$set": stats})
    except: pass

async def update_run_status_only(run_id: str, status: str):
    try: await runs_collection.update_one({"_id": ObjectId(run_id)}, {"$set": {"status": status, "finished_at": datetime.now(timezone.utc) if status == "finished" else None}})
    except: pass

async def add_result_to_run(run_id: str, result_type: str, line: str):
    field = "hit_lines" if result_type == "hit" else "custom_lines"
    try: await runs_collection.update_one({"_id": ObjectId(run_id)}, {"$push": {field: line}})
    except: pass
...
async def add_result_details_to_run(run_id: str, result_type: str, data: dict):
    ...
    try: await runs_collection.update_one({"_id": ObjectId(run_id)}, {"$push": {field: data}})
    except: pass
...
async def finish_and_clean_run(run_id: str, hits_url: str, custom_url: str):
    try:
        ...
        await runs_collection.update_one({"_id": ObjectId(run_id)}, {"$set": update_data, "$unset": { "hit_lines": "", "custom_lines": "", "hit_details": "", "custom_details": "" }})
    except: pass
...
async def create_invite_code(code: str):
    invite = {"code": code, "created_at": datetime.now(timezone.utc), "is_used": False, "used_by": None}
...
async def mark_invite_used(code: str, email: str):
    await invites_collection.update_one({"code": code}, {"$set": {"is_used": True, "used_by": email}})
...
async def set_user_needs_new_invite(invite_code: str):
    await users_collection.update_many({"current_invite_code": invite_code}, {"$set": {"needs_new_invite": True}})

async def reactivate_user(email: str, new_code: str):
    await users_collection.update_one({"email": email}, {"$set": {"needs_new_invite": False, "current_invite_code": new_code}})
