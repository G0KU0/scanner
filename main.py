...
# Globális változók a szálkezeléshez
user_connections = {}
...
# Konfiguráció
MAX_WORKERS = 60  # Sebesség optimalizáció miatt megemelt érték
...
async def lifespan(app: FastAPI):
    """A szerver indulásakor és leállásakor futó háttérfeladatok."""
    print("\n🚀 Hotmail Inboxer VIP - Szerver indítása...")

    # RENDER FIX: Proxy setup elindítása a háttérben, hogy ne blokkolja a port nyitását
    ...
    # Proxy frissítő ciklus (45 percenként)
    ...
    # Beragadt futások lezárása indításkor
    try:
...
def broadcast_to_user(user_id: str, message: str):
    """Valós idejű WebSocket üzenet küldése a felhasználónak."""
    with ws_lock:
...
async def get_me(current_user=Depends(get_current_user)):
    """Felhasználó zárolási állapotának lekérése."""
    return {
...
async def execute_checker(run_id: str, user_id: str, lines: list, keyword: str, num_threads: int):
    """Optimalizált checker végrehajtó szekvencia batching logikával."""
    ...
    def check_single(line):
        ...
        # AZONNALI STOP ELLENŐRZÉS a szál belépésekor
        with stop_lock:
        ...
        with lock:
            ...
            if result["status"] == "hit":
                hits += 1
                d = result["data"]
                lt = f"[HIT] {d['email']}:{d['password']} | Ország: {d['country']} | Levelek: {d['mails']}"
    ...
    # BATCH FELDOLGOZÁS a gyors stop érdekében
