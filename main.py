...
async def execute_checker(run_id: str, user_id: str, lines: list, keyword: str, speed: float = 0.3):
    ...
    for line in lines:
        ...
        while retry_count < MAX_RETRIES_PER_LINE:
            ...
            if result["status"] == "retry":
                ...
                broadcast_to_user(user_id, json.dumps({
                    ...
                    "text": f"⚠️ [RATE LIMIT] {email} – Várakozás {wait_time}s... ({retry_count}/{MAX_RETRIES_PER_LINE})"
                }))
        ...
        if result["status"] == "hit":
            ...
            bad += 1
            broadcast_to_user(user_id, json.dumps({"type": "log", "level": "bad", "text": f"[BAD] {email} ({result.get('reason', '')})"}))
        else: 
            retries += 1
            broadcast_to_user(user_id, json.dumps({"type": "log", "level": "warning", "text": f"⚠️ [ERROR] {email} ({result.get('reason', '')})"}))
    ...
    broadcast_to_user(user_id, json.dumps({"type": "log", "level": "finish", "text": f"[{st_text}] Befejezve! Hits: {hits} | Custom: {custom} | Bad: {bad} | Retries: {retries}"}))
